...
interface KitchenIslandProps extends FurnitureComponentProps {
  width?: string;
  items?: FoodItem[];
}
...
export default function KitchenIsland({ className, placement, width = "w-24", items = ["tomato", "cucumber"] }: KitchenIslandProps) {
  return (
    <FurnitureBase placement={placement} className={cn(width, "h-10 bg-amber-600 rounded-lg border-2 border-amber-700", className)}>
      {items.map((item, i) => {
